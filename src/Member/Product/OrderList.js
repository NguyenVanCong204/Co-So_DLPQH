...
function OrderList() {
  ...
  const [activeTab, setActiveTab] = useState("waiting"); // waiting, delivery, delivered
  ...
  const getOrders = async () => {
    try {
      ...
      if (error.response?.status === 401) {
        ...
        toast.error("Bạn đang đăng nhập với quyền Admin. Vui lòng đăng nhập lại với tài khoản Member để xem đơn hàng.");
      } else {
  ...
  const groupOrdersByCode = (orderList) => {
    ...
    orderList.forEach((order) => {
      // For orders without orderCode, use createdAt as grouping key
      ...
      <div key={orderCode} className="order-card">
        <div className="order-header">
          <div>
            ...
            <span className="order-date">{formatDate(firstOrder.createdAt)}</span>
          </div>
          <div className="order-status-badge">
            ...
            {status === 3 && <span className="badge cancelled" style={{backgroundColor: '#dc3545'}}>Đã hủy</span>}
          </div>
        </div>
        ...
        <div className="order-footer">
          <div className="order-total">
            ...
            <p>Phương thức thanh toán: {firstOrder.paymentMethod === "paypal" ? "PayPal" : "Tiền mặt (COD)"}</p>
          ...
          {status === 1 && (
            <button
              ...
              onClick={() => handleMarkAsDelivered(items[0]._id)}
            >
              Đã nhận hàng
            </button>
          )}
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="order-detail-section">
                ...
                <p><strong>Trạng thái:</strong> <span className="badge delivered">Đã giao hàng</span></p>
                <p><strong>Ngày đặt:</strong> {formatDate(selectedOrder.firstOrder.createdAt)}</p>
                <p><strong>Phương thức thanh toán:</strong> {selectedOrder.firstOrder.paymentMethod === "paypal" ? "PayPal" : "Tiền mặt (COD)"}</p>
                {selectedOrder.firstOrder.address && (
                  <p><strong>Địa chỉ giao hàng:</strong> {selectedOrder.firstOrder.address}</p>
                )}
              </div>
